            ...
            successful = DatabaseConnection.getInstance().unsafely(supabase -> {
                ...
                supabase.transaction(transaction -> {
                    ...
                    for (GeorgiaPacificLoadTender.GroupStopOffDetails210 stop : shipment.data().section2().groupStopOffDetails_210()) {
                        ...
                        Optional<GeorgiaPacificLoadTender.DateOrTime> scheduled = Objects.isNull(shipment.data().section1().dateOrTime()) ? Optional.empty() :
                        ...
                        Optional<GeorgiaPacificLoadTender.DateOrTime> _earliest = scheduled.isPresent() ? scheduled : Objects.isNull(shipment.data().section1().dateOrTime()) ? Optional.empty() :
                                stop.dateOrTime().stream().filter(i -> {
                        ...
                        Optional<GeorgiaPacificLoadTender.DateOrTime> _latest = scheduled.isPresent() ? scheduled : Objects.isNull(shipment.data().section1().dateOrTime()) ? Optional.empty() :
                                stop.dateOrTime().stream().filter(i -> {
